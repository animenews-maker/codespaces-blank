...
def generate_poster(
    ...
    Args:
        ...
        description (str): The description or tagline on the poster.
        logo_path (Optional[str]): Path to the logo image file (if any).
        rating (Optional[float]): Rating value to display (0-10 scale).
        studio (Optional[str]): Studio or network name to display.
        genre (Optional[str]): Genre for background theming and tagging.
        season (Optional[str]): Season for background theming and tagging.
        trailer_url (Optional[str]): URL for the trailer video (QR code will be generated).
        output_path (str): The file path to save the generated poster image.
        bg_color (tuple): Background color of the poster in RGB format.
        channel_name (Optional[str]): Discord channel name for tagging.
        post_type (Optional[str]): Type of post (e.g., "News", "Update") for tagging.
        timestamp (Optional[str]): Custom timestamp for the post, overrides automatic timestamp.

    Returns:
        str: The file path of the saved poster image.
    """
